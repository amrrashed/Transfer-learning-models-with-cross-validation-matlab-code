...
for fold_idx=1:num_folds
 ...
 labeltrain= countEachLabel(imdsTrain)
   ...
   % layersTransfer=googlenet('Weights','none');
    %clear net;
 % Number of categories %90.9% 5 layers removed sgdm 
...
%    [predicted_labels(test_idx),posterior(test_idx,:)] = classify(netTransfer,imdsTest);
    ...
    save(sprintf('ALEXNET_%d_among_%d_folds',fold_idx,num_folds),'netTransfer','test_idx','train_idx','labeltest','labeltrain');
    
    % Clearing unnecessary variables 
    clearvars -except fold_idx num_folds num_images predicted_labels posterior imds netTransfer layers;
